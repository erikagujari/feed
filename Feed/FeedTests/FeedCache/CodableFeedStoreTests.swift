...
class CodableFeedStoreTests: XCTestCase {
        ...
        sut.retrieve { firstResult in
            sut.retrieve { secondResult in
                ...
                case (.empty, .empty):
                    break
                default:
                    XCTFail("Expected retrieving twitce from empty cache to deliver same empty result, got \(firstResult) and \(secondResult) instead")
                }
                exp.fulfill()
            }
        }
    ...
    func test_retrieveAfterInsertingToEmptyCache_hasNoSideEffectsOnEmptyCache() {
        let sut = makeSUT()
    ...
    func test_retrieve_hasNoSideEffectsOnNonOnEmptyCache() {
        let sut = makeSUT()
        let feed = uniqueImageFeed().local
        let timestamp = Date()
        let exp = expectation(description: "Wait for cache retrieval")
        ...
        sut.insert(feed, timestamp: timestamp) { insertionError in
            ...
            sut.retrieve { firstResult in
                sut.retrieve { secondResult in
                    ...
                    default:
                        XCTFail("Expected retrieving twitce from non empty cache to deliver same found result with with feed \(feed) and timestamp \(timestamp), got firstResult \(firstResult) and secondResult \(secondResult) instead")
                    }
                    exp.fulfill()
                }
            }
