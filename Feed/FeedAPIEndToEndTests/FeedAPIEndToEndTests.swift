...
class FeedAPIEndToEndTests: XCTestCase {
    func test_endToEndTestServerGETFeedResult_matchesFixedTestAccountData() {
        ...
        case let .success(imageFeed)?:
            XCTAssertEqual(imageFeed.count, 8, "Expected 8 images in the last account feed")
    ...
    private func expectedImage(at index: Int) -> FeedImage {
        return FeedImage(id: UUID(uuidString: expectedId(at: index))!,
                        description: expectedDescription(at: index),
                        location: expectedLocation(at: index),
                        url: expectedImageUrl(at: index))
    }
